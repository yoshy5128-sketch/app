...
let gameSettings = {
    playerHP: 3,
    aiHP: 3,
    projectileSpeedMultiplier: 1.5, // デフォルト値を1.5に変更
    mgCount: 1,
    rrCount: 1,
    srCount: 1,
    sgCount: 1, // ショットガンを追加
    medikitCount: 0, // メディキットの出現数
    fieldState: 'reset',
    ...
    nightModeLightIntensity: 0.8, // ナイトモード時の外灯の強度
...
(function() {
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // projectile-speedスライドバーのイベントリスナー
        ...
        if (projectileSpeedSlider) {
            projectileSpeedSlider.addEventListener('input', () => { // inputイベントでリアルタイム更新
...
function loadSettings() {
    ...
    if (savedSettings) {
        ...
        if (parsedSavedSettings.nightModeLightIntensity === undefined) {
            parsedSavedSettings.nightModeLightIntensity = 0.8; // デフォルト値
        }
        if (parsedSavedSettings.medikitCount === undefined) {
            parsedSavedSettings.medikitCount = 0; // デフォルト値
        }
        ...
        if (document.getElementById('medikit-count')) document.getElementById('medikit-count').value = gameSettings.medikitCount; // メディキットを追加
...
const MEDIKIT_HEIGHT = 0.8; // 武器BOXと同じ高さに
...
function createMedikitPickup(position) {
    ...
    const boxMaterial = new THREE.MeshLambertMaterial({ color: 0xffffff }); // 白色
    const boxMesh = new THREE.Mesh(boxGeometry, boxMaterial);
    boxMesh.position.y = MEDIKIT_HEIGHT / 2; // グループ内でY座標を調整
    medikitGroup.add(boxMesh);

    // 2. 赤十字を作成 (2つのBoxGeometryを重ねる)
    const crossThickness = 0.25; // 十字の太さを調整
    const crossLength = MEDIKIT_HEIGHT * 0.8; // メディキットの高さに合わせて調整

    const crossGroup = new THREE.Group(); // 十字全体をグループ化

    // 縦棒
    const verticalBarGeometry = new THREE.BoxGeometry(crossThickness, crossLength, crossThickness);
    const crossMaterial = new THREE.MeshLambertMaterial({ color: 0xff0000 }); // 赤色
    const verticalBar = new THREE.Mesh(verticalBarGeometry, crossMaterial);
    verticalBar.position.y = 0; // crossGroup内での相対Y座標
    ...
    horizontalBar.position.y = 0; // crossGroup内での相対Y座標
    crossGroup.add(horizontalBar);

    // 十字グループをメディキットBOXのX+面に配置し、面に密着させる
    const crossGroup1 = crossGroup.clone(); // 十字グループを複製
    crossGroup1.position.set(MEDIKIT_WIDTH / 2 - crossThickness / 2, MEDIKIT_HEIGHT / 2, 0); // X+面に移動
    crossGroup1.rotation.y = Math.PI / 2; // Y軸周りに90度回転
    medikitGroup.add(crossGroup1);

    // 十字グループをメディキットBOXのX-面に配置し、面に密着させる
    const crossGroup2 = crossGroup.clone(); // 十字グループを複製
    crossGroup2.position.set(-MEDIKIT_WIDTH / 2 + crossThickness / 2, MEDIKIT_HEIGHT / 2, 0); // X-面に移動
    crossGroup2.rotation.y = -Math.PI / 2; // Y軸周りに-90度回転
    medikitGroup.add(crossGroup2);

    // BOXと十字のグループ全体のY座標を調整 (地面からの高さ)
    medikitGroup.position.copy(position);
    medikitGroup.position.y = -FLOOR_HEIGHT; // グループ全体が地面に接するように

    // BOXと十字のグループ全体のY座標を調整 (地面からの高さ)
    medikitGroup.position.copy(position);
    medikitGroup.position.y = -FLOOR_HEIGHT + 0.01; // グループ全体が地面に接するように + わずかに浮かせます

    // userDataをグループに設定
    medikitGroup.userData = { type: 'medikitPickup' };

    scene.add(medikitGroup);
    weaponPickups.push(medikitGroup); // 一旦weaponPickupsに含めて管理
    console.log(`[DEBUG] Medikit created at position: (${position.x.toFixed(2)}, ${position.y.toFixed(2)}, ${position.z.toFixed(2)})`);
    return medikitGroup;
}
...
function aiCheckPickup(ai) {
    for (let i = weaponPickups.length - 1; i >= 0; i--) {
        ...
        if (pickup.userData.type === 'medikitPickup') continue; // AIはメディキットを拾わない
...
function startGame() {
    ...
    if (gameSettings.nightModeEnabled) {
        ...
        streetLights.forEach(light => {
            ...
            if (pointLight) pointLight.intensity = gameSettings.nightModeLightIntensity; // 設定値を使用
        });
    } else {
...
function resetWeaponPickups() {
    ...
    createWeaponPickups(); // 通常の武器ピックアップを再作成
}
...
function restartGame() {
    ...
    if (gameSettings.gameMode === 'arcade' && gameSettings.medikitCount > 0) {
        console.log(`[DEBUG] Arcade Mode: Attempting to create ${gameSettings.medikitCount} medikits.`);
...
function respawnAI(ai) {
    ...
    // プレイヤーから最も遠い安全な位置を探す
    let farthestPosition = null; // 安全な場所が見つからなかった場合のためにnullで初期化
    let maxDistance = -Infinity;

    const NUM_RESPAWN_CANDIDATES = 50; // 候補の数を増やす
    ...
    // 安全な場所が見つからなかった場合のフォールバック
    if (!farthestPosition) {
        console.warn("Could not find a safe respawn position. Spawning at center as a fallback.");
                ...
                if (playerCollisionBox.intersectsBox(pickupBoundingBox)) {
                    if (pickup.userData.type === 'weaponPickup') {
                        ...
                        continue; 
                                                            ...
                                                            console.log(`[DEBUG] Player near medikit.`);
                                                            console.log(`[DEBUG] Player Pos: (${player.position.x.toFixed(2)}, ${player.position.y.toFixed(2)}, ${player.position.z.toFixed(2)})`);
                                                            console.log(`[DEBUG] Medikit Pos: (${pickup.position.x.toFixed(2)}, ${pickup.position.y.toFixed(2)}, ${pickup.position.z.toFixed(2)})`);
                                                            console.log(`[DEBUG] Distance to Medikit: ${player.position.distanceTo(pickup.position).toFixed(2)}`);
                                                            console.log(`[DEBUG] Player BBox: min(${playerCollisionBox.min.x.toFixed(2)}, ${playerCollisionBox.min.y.toFixed(2)}, ${playerCollisionBox.min.z.toFixed(2)}) max(${playerCollisionBox.max.x.toFixed(2)}, ${playerCollisionBox.max.y.toFixed(2)}, ${playerCollisionBox.max.z.toFixed(2)})`);
                                                            console.log(`[DEBUG] Medikit BBox: min(${pickupBoundingBox.min.x.toFixed(2)}, ${pickupBoundingBox.min.y.toFixed(2)}, ${pickupBoundingBox.min.z.toFixed(2)}) max(${pickupBoundingBox.max.x.toFixed(2)}, ${pickupBoundingBox.max.y.toFixed(2)}, ${pickupBoundingBox.max.z.toFixed(2)})`);
                                            
                                                            if (playerHP !== Infinity) {
                                                                playerHP++; // HPを1増加
                                                                playerHPDisplay.textContent = `HP: ${playerHP}`;
                                                                console.log(`[DEBUG] Player picked up medikit. New HP: ${playerHP}`); // デバッグログ追加
                                                            }                                    // 音声再生
                                    const setSound = document.getElementById('setSound');
                                    if (setSound) setSound.cloneNode(true).play();
                ...
                if (ai.currentWeapon === WEAPON_SR && ai.ammoSR > 0) {
                    ...
                    if (!findAndTargetWeapon(ai)) {
                                            if (isAISeen) {
                                                ...
                                                const effectiveHideDuration = HIDE_DURATION * (1.0 + (1.0 - ai.aggression) * 1.5); // aggressionに基づいて調整
